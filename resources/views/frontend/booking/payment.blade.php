...
@section('content')
    ...
    {{-- MODIFIKASI: Ambil Status Production & Client Key dari SettingsHelper (Database) --}}
    @php
        // Cek mode production dari database (1 = Production, 0 = Sandbox)
        $isProduction = \App\Helpers\SettingsHelper::get('midtrans_is_production') == 1;
        
        // Ambil Client Key dari database, jika kosong fallback ke config .env
        $clientKey = \App\Helpers\SettingsHelper::get('midtrans_client_key');
        if (empty($clientKey)) {
