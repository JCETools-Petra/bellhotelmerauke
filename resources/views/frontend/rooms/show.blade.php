...
@section('content')
    ...
    <div class="bg-gray-900 pt-24 pb-12 relative overflow-hidden">
        ...
        <div class="container mx-auto px-4 relative z-10">
            ...
            {{-- MODIFIKASI: Harga Dinamis di Header --}}
            <div class="text-yellow-500 text-lg md:text-xl font-medium">
                @if(Auth::check() && in_array(Auth::user()->role, ['admin', 'affiliate']))
                    ...
                    <span class="text-sm text-gray-300">/ malam (Affiliate)</span>
                @else
                    {{-- Tampilan Tamu Biasa --}}
                    <span>
                        Rp {{ number_format($room->calculated_public_price, 0, ',', '.') }}
                    </span>
                    <span class="text-sm text-gray-300">/ malam</span>
                @endif
            </div>
            {{-- AKHIR MODIFIKASI --}}
                ...
                <div class="lg:col-span-1">
                    <div class="sticky top-24 space-y-6">
                        ...
                        <div class="bg-white rounded-2xl shadow-lg border-t-4 border-yellow-500 overflow-hidden">
                            <div class="p-6">
                                ...
                                <form action="{{ route('bookings.store') }}" method="POST" id="bookingForm">
                                    ...
                                    {{-- MODIFIKASI: Input Hidden Harga menggunakan variable controller yang baru --}}
                                    <input type="hidden" id="room_price" 
                                           value="{{ (Auth::check() && in_array(Auth::user()->role, ['admin', 'affiliate'])) ? $room->calculated_affiliate_price : $room->calculated_public_price }}">
                                    {{-- AKHIR MODIFIKASI --}}
                                    ...
                                    <button type="submit" class="w-full mt-6 bg-gray-900 hover:bg-yellow-500 text-white hover:text-gray-900 font-bold py-3.5 rounded-lg transition-all duration-300 shadow-lg transform hover:-translate-y-1">
                                        {{-- MODIFIKASI: Gunakan SettingsHelper untuk cek booking method --}}
                                        @if(\App\Helpers\SettingsHelper::get('booking_method', 'direct') == 'direct')
                                            Lanjut Pembayaran
                                        @else
                                            Pesan via WhatsApp
                                        @endif
                        ...
                        <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100 text-center">
                            ...
                            {{-- MODIFIKASI: Gunakan SettingsHelper untuk No WA --}}
                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', \App\Helpers\SettingsHelper::get('contact_phone') ?? '6281234567890') }}" target="_blank" class="text-green-600 font-bold hover:underline flex items-center justify-center gap-2">
