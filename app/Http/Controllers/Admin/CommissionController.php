...
class CommissionController extends Controller
{
    ...
    public function index()
    {
        ...
        $affiliates = Affiliate::with('user')
            ...
            ->paginate(50); // <--- UBAH DARI 15 JADI 50 (atau angka lain sesuka Anda)
    ...
    public function show(Affiliate $affiliate)
    {
        ...
        $commissions = Commission::where('affiliate_id', $affiliate->id)
            ->where('status', 'unpaid')
            // Hapus filter bulan ini jika ingin melihat SEMUA yang belum dibayar
            // ->whereMonth('created_at', now()->month) 
            // PERBAIKAN: Load relasi room DAN miceKit
            ->with(['booking.room', 'booking.miceKit']) 
            ->get();
